...
// get the updated adjcency list whose vertices are bigger than current vertex
vlist*   get_new_neighbors(vlist*, vid_t);
...
clique_compute( uGraph *g,
    ...
    for(vlist::const_iterator iter = allvtx->begin();
        ...
        //vlist *tmp_cand = get_new_neighbors(g->adjlist(*iter), *iter);
        task_t *tmp = new task_t( tmp_cand, set_insert_copy(*iter), 0);
    ...
    */
    ...
    for( vlist::const_iterator iter = t->cand->begin();
        ...
        if(candidate->size() != 0){
            ...
            task_t *tmp  = new task_t(candidate, newc, 0);
            tasks.insert_tail(tmp);
        } else {
...
vlist*
get_new_neighbors(vlist* vl, vid_t v){
    
    vlist *res = new vlist();
    for(vlist::iterator iter = vl->begin();
        iter != vl->end();
        ++iter){
